...
- 매칭 실행: 관리자 수동 실행 (스케줄러 자동화는 배포 환경 구축 후)
- 매칭 결과: **인앱 결과 화면**으로 전달. 사업자등록 완료 후 카카오 알림톡 병행
- 표시 내용: 상대방 이름·학교 + 기입한 연락처(인스타/카톡ID/전화번호 중 1개 이상)
- 프로필 사진·자기소개는 결과 화면에 표시하지 않음 (노출 최소화)

### 5.2 매칭 알고리즘
- **설계 완료** (2026-08-21) → `2026-08-21-matching-algorithm-design.md`
- 요약: 가치관 설문 기반 궁합 점수(양방향 min) + 절대질문 하드필터 + 전체 최적 매칭
- 미매칭자는 다음 라운드에서 이월 보너스를 받음 / 재매칭은 영구 금지
- 대학별 가중치는 관리자 화면에서 설정 (이벤트용)

### 5.3 매칭 점수 수정자 (게임 연동)
- 오작교: 같은 쌍을 지목한 지목자 1명 → +33점 / 2명 → +66점 / 3명 이상 → 매칭 보장
- 붉은 실 상호 입력 시: 매칭 보장
- **절대질문이 보장보다 우선한다** — 절대질문을 어기는 페어는 보장에서 탈락
- 게임 대상 식별: 이름+학교+학번으로 유일하게 특정될 때만 적용 (동명이인이면 무시)
- 중복 매칭 방지: 과거에 매칭된 적 있는 페어는 영구 제외
- (점수 적용은 매칭 알고리즘 영역 — 게임은 지목/입력 저장·집계만 담당)
...
| 회원가입 | 기본 정보 입력(대학 선택, 자유 입력 아님) + 연락처(인스타/카톡ID/전화번호 중 1개 이상 필수) + 학번(선택 입력) + 학생증 업로드 |
| 인증 대기 | 관리자 승인 대기 화면 |
| 로그인 | 이메일+비밀번호 |
| 가치관 설문 | 매칭 알고리즘용 설문 |
...
GET  /survey/questions            -- 설문 문항 카탈로그 (프론트 렌더용)

POST /admin/match-rounds/{id}/run -- 관리자: 매칭 실행
GET  /me/match                    -- 내 최근 매칭 결과 (인앱 결과 화면)

GET    /admin/university-weights       -- 관리자: 대학 가중치 목록
POST   /admin/university-weights       -- 관리자: 대학 가중치 생성
PUT    /admin/university-weights/{id}  -- 관리자: 대학 가중치 수정
DELETE /admin/university-weights/{id}  -- 관리자: 대학 가중치 삭제
...
|---|---|
| 매칭 요일/시간 | 팀 결정 필요 |
| ~~매칭 알고리즘 상세~~ | ✅ 설계 완료 (2026-08-21) — `2026-08-21-matching-algorithm-design.md` |
| ~~학번(입학년도) 필드~~ | ✅ 완료 (2026-09-05) — `2026-09-04-signup-data-integrity-design.md` |
| 16개 대학 목록 | 관리 방식(admin CRUD)은 확정. 실제 목록만 팀 내부 선정 필요 — 목록이 비어 있으면 가입 자체가 불가능하다 |
