...
| 지우기 (재시도) | `api/rounds.py` — `update_round`, `reset_round` |
...
일정 수정(`update_round`)으로 미래 시각을 다시 잡거나 reset(`reset_round`)으로 멈춘 라운드를
되돌리는 것도 관리자 입장에서는 "다시 시도하겠다"는 뜻이다. 재시도 금지 필터
(`last_error is None`)가 실행 조건에 박혀 있으므로, 이 두 경로가 지우지 않으면 라운드는
스케줄러에서 영구 제외된다 — 카드에는 미래 시각인데 놓침 문구가 남고, 자동 실행은 다시 오지 않는다.
